...
/**
 * Call all observers for a given object
...
function $set(object, property, value) {
  if (object && object[OBSERVER_PROPERTY]) {
    var exists = ownProperty.call(object, property),
      oldVal = object[property],
      sz1 = object.length,
      chgset = null;
...
/**
 * Deletes a value in a JSON object (including arrays) and automatically notifies all observers of the change
 */
function $delete(object, property) {
  if (object && object[OBSERVER_PROPERTY]) {
    ...
    if (exists) {
      chgset = [changeDesc(object, property, object[property], oldVal, "deleted")];
    }
    if (object.constructor === Array) {
      var sz2 = object.length;
      if (sz1 !== sz2) {
...
function unobserve(object, callback) {
  ...
  if (res.length === 0) {
    delete object[OBSERVER_PROPERTY]; // TODO remove delete to benefit from v8 optimizations?
  } else {
